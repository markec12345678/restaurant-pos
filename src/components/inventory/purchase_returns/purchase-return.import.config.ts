...
export type PurchaseReturnLinePayload = {
  item: SelectOption;
  quantity: number;
  comments?: string;
...
export type PurchaseReturnLineAppend = (line: PurchaseReturnLinePayload) => void;
...
export function createPurchaseReturnImportConfig({
  ...
  update: (index: number, line: PurchaseReturnLinePayload) => void;
  getLines: () => any[];
}): ImportConfiguration {
  ...
  return {
    ...
    onImportRow: async (record: ImportRecord, ctx: ImportRowContext) => {
      ...
      const payload: PurchaseReturnLinePayload = {
