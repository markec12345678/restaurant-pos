...
export function useStockTransferList(
  initialPage = 0,
  initialPageSize = 10
): UseApiResult<SettingsData<StockTransfer>> & {
  listFilters: StockTransferListFilters;
  setListFilters: (filters: StockTransferListFilters) => void;
} {
  const db = useDB();
  const {isConnected} = useDatabase();
  const [page, setPage] = useState(initialPage);
  const [pageSize, setPageSize] = useState(initialPageSize);
  const [listFilters, setListFilters] = useState<StockTransferListFilters>({});
  const [filters] = useState<string[]>([]);
  const [sorts, setSorts] = useState<string[]>(["created_at DESC"]);

  const queryKey = ["stock_transfers", page, pageSize, listFilters];

  const {data, isLoading, isFetching, isError, error, refetch} = useQuery({
    queryKey,
    queryFn: () =>
      listStockTransfers(db, {page, pageSize, filters: listFilters}),
